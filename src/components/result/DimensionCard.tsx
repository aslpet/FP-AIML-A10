...
export function DimensionCard({
  ...
  const tintClass = score >= 4 ? "score-strong" : score <= 2 ? "score-weak" : "";
  ...
  return (
    <motion.div
      ...
      className={`card-std ${tintClass}`}
    >
      <div className="flex items-center justify-between">
        <div>
          <p className="font-bold text-ink">{label}</p>
          <p className="text-[11px] text-ink/35">Bobot {Math.round(weight * 100)}%</p>
        </div>
        <span className="text-lg font-extrabold tabular-nums" style={{ color }}>
          {score}
          <span className="text-sm text-ink/25">/5</span>
        </span>
      </div>
      <div className="my-3">
